...
def monotonic_gain_loss(y_hat, u, scaler_y, scaler_u, y, evaluate = 'meal', cumulative_window = 12*1.5, horizon=12*1.5, 
                        aggressive_coeff=0.7, CR = 1.0, CF = 1.0, G_bar = torch.tensor([110]), return_debug=False, type_preprocess_insulin = 'sum', loaded_parameters=None, use_low_pass_I_on_prior = False, l_filter = None):
    """
    Enforces positive gain over a horizon for positive inputs (VECTORIZED).
    ...
    if evaluate == 'meal':
        ...
        if use_low_pass_I_on_prior and l_filter is not None:
            l_filter.reset(prior_delta_y[:,0,:])
            prior_delta_y = l_filter(prior_delta_y)
...
class ClosedLoopSystem_SSM(nn.Module):
    ...
    def __init__(self, system_model, controller, negative: bool = False, u_noisy: bool = True):
        super().__init__()
    ...
    def forward(self, u_ext, time):
        ...
        #Compute next state and output
        control_u, _, control_u_noisy, _ = self.controller.forward(self.y_prev.squeeze(dim=1), time)
        
        if self.u_noisy:
                control_u = control_u_noisy
    ...
    def run(self, u_ext, time, y_t0=None):
        ...
        self.system_model.reset()
        self.system_model.reset_f(-y_t0)
        ...
        for i, t in enumerate(time[0,:,0]):
            ...
            control_u, _, control_u_noisy,_ = self.controller.forward(self.y_prev.squeeze(dim=1), t_batch)  # Compute control input
            # I, I_rwgn, I_sat, R
