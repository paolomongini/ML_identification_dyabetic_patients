...
class ClosedLoopSystem_SSM(nn.Module):
    ...
    def __init__(self, system_model, controller, negative: bool = False, explosion_threshold=1000):
        super().__init__()
        ...
        self.register_buffer('y0_prev', None)
        self.register_buffer('y1_prev', None)
    ...
    def _check_explosion(self, y, y0, y1, step):
        """
        Check if any value exceeds explosion threshold.
        Raises exception if explosion detected.
        
        Args:
            y, y0, y1: output tensors
            step: current step in the loop
        
        Raises:
            RuntimeError: if explosion detected
        """
        y_max = torch.abs(y).max().item() if y is not None else 0
        y0_max = torch.abs(y0).max().item() if y0 is not None else 0
        y1_max = torch.abs(y1).max().item() if y1 is not None else 0
        
        if y_max > self.explosion_threshold or y0_max > self.explosion_threshold or y1_max > self.explosion_threshold:
            raise RuntimeError(
                f"[EXPLOSION DETECTED] at step {step}: "
                f"y_max={y_max:.2f}, y0_max={y0_max:.2f}, y1_max={y1_max:.2f} "
                f"(threshold={self.explosion_threshold})"
            )
    ...
    def run(self, u_ext, time, y_t0=None):
        ...
        for i, t in enumerate(time[0,:,0]):
            ...
            self._check_explosion(y, y0, y1, step=i)
